...
router.post('/generate-section-summaries', async (req, res, next) => {
  try {
    ...
    const { generateClassQuestionsSummary } = await import('../services/insightService.js');
    for (const cls of allClasses.rows) {
      await generateClassQuestionsSummary(cls.id_class);
    }
...
// GET /ai/questions-summary - Get the latest AI summary of student questions
router.get('/questions-summary', async (req, res, next) => {
  try {
    const classId = parseInt(req.query.classId as string);
    if (!classId) return res.status(400).json({ error: 'classId is required' });

    const result = await db.query<any>(
      `SELECT * FROM class_questions_summary 
       WHERE id_class = ? 
    ...
    if (result.rows.length === 0) {
      return res.json({ success: false, message: 'No questions summary found' });
    }

    const row = result.rows[0];
    res.json({
      success: true,
      summary: row.questions_summary,
      topDoubts: typeof row.top_doubts === 'string' ? JSON.parse(row.top_doubts) : row.top_doubts,
      avgFrustration: row.avg_frustration,
      totalQuestions: row.total_questions,
      generatedAt: row.generated_at
    });
  } catch (error: any) {
    console.error('Questions summary fetch error:', error);
    res.status(500).json({ error: error.message });
  }
});
