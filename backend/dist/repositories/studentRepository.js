...
export async function getStudentTopicMastery(userId, topicId) {
    const result = await db.query(`SELECT score, ai_summary 
     FROM student_topic 
     WHERE id_user = ? AND id_topic = ?`, [userId, topicId]);
...
export async function getTopicSessionHistory(userId, topicId) {
    const result = await db.query(`SELECT cst.id_class, c.name_session as class_name, cst.score, c.start_time as date
     ...
     ORDER BY c.start_time DESC`, [userId, topicId]);
    return result.rows;
}
export async function getHistoricalRecordsForTopic(userId, topicId) {
    ...
    const [sessions, quizzes] = await Promise.all([
       ...
       ORDER BY c.start_time DESC`, [userId, topicId]),
        ...
        db.query(`SELECT q.name as quiz_name, qa.total_score as score, qa.finished_at
       FROM quiz_attempt qa
       INNER JOIN quiz q ON q.id_quiz = qa.id_quiz
       INNER JOIN quiz_topic qt ON qt.id_quiz = q.id_quiz
       WHERE qa.id_student = ? AND qt.id_topic = ? AND qa.finished_at IS NOT NULL
       ORDER BY qa.finished_at DESC`, [userId, topicId])
    ]);
    return {
...
export async function updatePermanentTopicSummary(userId, topicId, summary) {
